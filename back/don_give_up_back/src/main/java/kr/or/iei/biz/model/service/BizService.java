...
public class BizService {
	...
	/*
	 * public ArrayList<BizFile> selectBizFileList(int bizNo) { return
	 * dao.selectBizFileList(bizNo); }
	 */
	...
	public int deleteBiz(int bizNo) {
		return dao.deleteBiz(bizNo);
	}
	...
	public int updateBiz(Biz biz) {
	    ...
	    int[] deletedPlanNosArr = biz.getDeletedPlanNos();
	    List<Integer> deletedPlanNosList = new ArrayList<>();
	    if (deletedPlanNosArr != null && deletedPlanNosArr.length > 0) {
	        for (int planNo : deletedPlanNosArr) {
	            deletedPlanNosList.add(planNo);
	        }
	        // (1) bizPlan 삭제 처리
	    ...
	    List<BizPlan> newBizPlanList = biz.getBizPlanList();
	    if (newBizPlanList != null) {
	        for (BizPlan plan : newBizPlanList) {
	            if (plan.getPlanNo() == 0) {
	    ...
	    // (4) 기부 사업 테이블 업데이트
	    int bizResult = dao.updateBiz(biz);
